...
#if defined(NDEBUG)
# error "Osizcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x01ba819dc72378fb4a98e669fb54963943aa3dff760a2aecfe774e6d8ee2c58d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Osizcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Osizcoin Signed Message:\n";
...
{
    // Osizcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 5000); // Osizcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Osizcoin: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Osizcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Osizcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Osizcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb368925976a91aa002c47d27b56170e2902042c85252bcc7a25cd623e2fbc85e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Osiz Technologies found in the year of 2010";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1528873521;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089320439;

        if (fTestNet)
        {
            block.nTime    = 1528873482;
            block.nNonce   = 386015990;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc56996411531c07e43a7cec80e0dd617532b213268863e440174fa5978ce1824"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbc, 0xd5 }; // Osizcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OsizcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OsizcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OsizcoinMiner(CWallet *pwallet)
{
    printf("OsizcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("osizcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OsizcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OsizcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OsizcoinMiner, pwallet));
}
